...
export const codexLunaSolPolicy: RouterPolicy = {
  ...
  signals: [
    {
      ...
      pattern: '(认真|仔细|深入|深度|想清楚|高质量|最好(?:的)?模型|最强|不要敷衍|strict(?:ly)? review|think\s+carefully|high[ -]?quality|best\s+model)',
      weight: 90,
      forceUpgrade: true
    },
    {
      ...
      pattern: '(省额度|节省额度|简单回答|简短回答|不用想太多|低成本|save\s+(?:tokens?|quota)|keep\s+it\s+simple)',
      weight: -45
    },
    {
      id: 'high-impact',
      reasonCode: 'high_impact',
      pattern: '(删除|覆盖|清空|迁移|批量.*(?:改|写|重命名)|凭据|密码|token|api\s*key|权限|安全|隐私|生产环境|上线|部署|服务器|数据库|备份|恢复|回滚|支付|合同|医疗|药物|delete|overwrite|migrat|credential|permission|security|privacy|production|deploy|database|backup|restore)',
      weight: 45
    },
    {
  ...
  simpleRequestPatterns: [
    '^(你好|嗨|hello|hi|谢谢|感谢|好的|好|行|可以|收到|知道了|嗯|哦|翻译[:：]?|润色[:：]?|格式化[:：]?|任务完成了吗|下载好了吗)[？?！!。\s\S]{0,32}$'
  ],
